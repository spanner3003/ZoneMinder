...
extern "C" void registerPlugin(PluginManager &PlM, string sPluginName) {
  PlM.getImageAnalyser().addDetector(
    unique_ptr<Detector>(new FaceDetectorPlugin(sPluginName))
  );
}
...
bool FaceDetectorPlugin::checkZone(Zone *zone, const Image *zmImage) {
  ...
  Image *pMaskImage = new Image(zmImage->Width(), zmImage->Height(), 1, 1);
  ...
  if ( tempZmImage->Colours() == 1 ) {
    ...
    unsigned char *buffer = tempZmImage->WriteBuffer(zmImage->Width(), zmImage->Height(), zmImage->Colours(), zmImage->SubpixelOrder());
  ...
  for (vector<CvRect>::iterator it = foundObjects.begin(); it < foundObjects.end(); ++it) {
    ...
    int x1 = int(it->x/m_fImageScaleFactor);
    int x2 = int((it->x + it->width)/m_fImageScaleFactor);
    int y1 = int(it->y/m_fImageScaleFactor);
    int y2 = int((it->y + it->height)/m_fImageScaleFactor);
  ...
  Image *hlZmImage = pMaskImage->HighlightEdges(alarm_colour, pMaskImage->Colours(), pMaskImage->SubpixelOrder(), &zone_polygon.Extent());
